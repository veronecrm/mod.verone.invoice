...
<div class="page-header">
    <div class="page-header-content">
        <div class="page-title">
            <h1>
                ...
                <?php echo $invoice->getId() ? $app->t('invoiceEdit') : $app->t('invoiceNew'); ?><small> / {{ t('invoiceTypeCorrection') }}</small>
            </h1>
        </div>
    ...
    <div class="breadcrumb-line">
        <ul class="breadcrumb">
            ...
            @if $invoice->getId()
                <li class="active"><a href="{{ createUrl('Invoice', 'CorrectionInvoice', 'edit', [ 'id' => $invoice->getId() ]) }}">{{ t('invoiceEdit') }}</a></li>
            @else
...
<div class="container-fluid">
    <div class="row">
        <div class="col-md-12">
            <form action="{{ createUrl('Invoice', 'CorrectionInvoice', $invoice->getId() ? 'update' : 'save') }}" method="post" id="form" class="form-validation">
                <input type="hidden" name="id" value="{{ $invoice->getId() }}" />
                <input type="hidden" name="correctionNumber" id="correctionNumber" value="{{ $invoice->getCorrectionNumber() }}" />
                <input type="hidden" name="correctionDate" id="correctionDate" value="{{ $invoice->getCorrectionDate() }}" />
                                        ...
                                        <div class="container-fluid">
                                            <div class="row">
                                                ...
                                                <div class="col-md-6">
                                                    ...
                                                    <div class="form-group">
                                                        <label for="correctionReason" class="control-label">{{ t('invoiceCorrectionReason') }}</label>
                                                        <select name="correctionReasonPredefined" id="correctionReasonPredefined" class="form-control">
                                                            <option value="0">{{ t('invoiceCorrectionReasonType0') }}</option>
                                                            <option value="{{ t('invoiceCorrectionReasonType1') }}">{{ t('invoiceCorrectionReasonType1') }}</option>
                                                            <option value="{{ t('invoiceCorrectionReasonType2') }}">{{ t('invoiceCorrectionReasonType2') }}</option>
                                                            <option value="{{ t('invoiceCorrectionReasonType3') }}">{{ t('invoiceCorrectionReasonType3') }}</option>
                                                            <option value="{{ t('invoiceCorrectionReasonType4') }}">{{ t('invoiceCorrectionReasonType4') }}</option>
                                                            <option value="{{ t('invoiceCorrectionReasonType5') }}" selected="selected">{{ t('invoiceCorrectionReasonType5') }}</option>
                                                        </select>
                                                        <textarea class="form-control auto-grow hidden" id="correctionReason" name="correctionReason">{{ $invoice->getCorrectionReason() }}</textarea>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                                                ...
                                                                <div class="cnt">
                                                                    <div class="container-fluid">
                                                                        <div class="row">
                                                                            <div class="col-md-6">
                                                                                <div class="form-group">
                                                                                    <label for="product_{{ $item->getId() }}_unitPriceNet" class="control-label">{{ t('invoiceUnitPriceNet') }}</label>
                                                                                    <div class="input-group">
                                                                                        <input class="form-control patt-product-unitPriceNet" type="text" id="product_{{ $item->getId() }}_unitPriceNet" name="product[{{ $item->getId() }}][unitPriceNet]" value="{{ $item->getUnitPriceNet() }}" />
                                    ...
                                    <div class="step-inn">
                                        <div class="invoice-preview">
                                                ...
                                                <tbody>
                                                    <tr>
                                                        ...
                                                        <td style="vertical-align:top;text-align:right;width:25%;">
                                                            ...
                                                            <span class="date-payment">{{ t('invoicePaymentDate') }}: <span></span></span><br />
                                                            <span>---------------------------------</span><br />
                                                            <span class="date-correction-number">{{ t('invoiceCorrectionNumber') }}: <span></span></span><br />
                                                            <span class="date-correction-date">{{ t('invoiceCorrectionDate') }}: <span></span></span>
                                                        </td>
                                                    </tr>
                                                </tbody>
                                            </table>
                                            ...
                                            <div class="correction-reason">
                                                <span style="text-align:left;font-weight:normal;font-size:12px;margin:0 0 2px 0;padding:0;color:#000;font-weight:bold;">Powód korekty:</span> &nbsp; <i style="font-size:11px;"></i>
                                            </div>
                                            <table>
                                                <tbody>
                                                    <tr>
...
<script>
    ...
    var beforeNextStep = function(step) {
        if(step == 1)
        {
            ...
            $('#releaseDate, #sellDate, #paymentDate, #paymentDays, #headline, #correctionReason').each(function() {
        ...
        if(step == 4)
        {
            ...
            $('.correction-reason i').text($('#correctionReason').val());
            $('.date-correction-number span').text($('#correctionNumber').val());
            $('.date-correction-date span').text($('#correctionDate').val());
    ...
    $(function() {
        ...
        $('#correctionReasonPredefined').change(function() {
            if($(this).val() == 0)
            {
                $(this).addClass('hidden');
                $('#correctionReason').removeClass('hidden').trigger('focus');
            }
            else
            {
                $('#correctionReason').val($(this).val());
            }
        });
        ...
        *****************************/
        ...
        APP.FormValidation.bind('form', '#correctionReason', {
            validate: function(elm, val) {
                if(jQuery.trim(val) == '' && $('#correctionReasonPredefined').val() == 0)
                    return false;
                else
                    return true;
            },
